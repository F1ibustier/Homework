from math import pi, sqrt# Импортируем модуль math для математических операций, таких как вычисление площади и корня.
...
class Circle(Figure):
    ...
    def __init__(self, color, circumference):
        ...
        self.__radius = circumference / (2 * pi)  # Рассчитываем радиус по длине окружности
        self.set_sides(circumference)  # Устанавливаем длину окружности как сторону

    def get_square(self):
        """Возвращает площадь круга."""
        return pi * (self.__radius ** 2)  # Площадь круга: π * r^2
